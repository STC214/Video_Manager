...
- 扫描阶段使用不确定进度条；Dry-run 使用目标校验、计划生成、空目录预览和 TSV 导出四阶段确定进度；移动和撤销阶段使用按文件数计算的确定进度条。
...
- UI 日志最新消息置顶并保留最近 200 行，文件日志保持时间正序；日志和预览更新期间暂停控件重绘。
...
- 跨盘复制完成后再次复核源文件大小和修改时间，目标修改时间写入成功后才删除源文件。
- 配置容量不足时自动增加第 1 层目录数量，并保持每叶文件数不变；不会仅因容量估算不足禁用正式移动。
- 普通路径和 Windows 长路径前缀统一规范化，用于同路径校验、目标树排除和清理保护。
- 最近撤销清单启动时后台验证；空清单、缺失清单和零移动诊断清单不会启用或覆盖有效撤销状态。
