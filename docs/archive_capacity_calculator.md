...
容量计算器中的“不足”用于提示配置规模。实际生成 Dry-run 时，规划器会保持每个叶目录的文件数不变，并自动增加第 1 层目录数量以容纳全部文件，不会把容量不足作为阻断正式移动的错误。

界面的容量结果仍显示原配置最大容量和缺少容量，同时明确提示 Dry-run 将自动扩展第 1 层。Dry-run 预览和日志会显示原容量、扩容后的容量以及实际采用的第 1 层目录数。
